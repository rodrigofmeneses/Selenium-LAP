...
class Preencher():
    ...
    def __init__(self, webdriver, path_dados, intervalo=(10,15), planilha='Plan SPG'):
        ...
        self.dados_montanteA = self._segmentar_dados(6, 13)
        self.dados_montanteB = self._segmentar_dados(13, 25)
        self.dados_montanteC = self._segmentar_dados(25, 27)
        self.dados_hora_extra = self._segmentar_dados(27, 33)
        self.dados_viagem = self._segmentar_dados(33, 38)
    ...
    def _preencher_demais_funcionarios(self, func):
        ...
        fdi.preencher_montante_A(self.dados_montanteA)
        fdi.ir_para_montanteB()
        fdi.preencher_montante_B(self.dados_montanteB)
        fdi.ir_para_montanteC()
        fdi.preencher_montante_C(self.dados_montanteC)
        fdi.ir_para_provisionamento()
        fdi.preencher_provisionamento_hora_extra(self.dados_hora_extra)
        fdi.preencher_provisionamento_viagem(self.dados_viagem)
        fdi.fechar()
    ...
    def _segmentar_dados(self, i, j):
        return self.dados[self.dados.columns[i:j]]
