...
class PreencherFatura:
    ...
    def __init__(
        self, webdriver, caminho_dados, intervalo_funcionarios, nome_planilha
    ):
        self.webdriver = webdriver
        self.pagina = PageFatura(self.webdriver)
        self.dados = self._carregar_dados(
            caminho_dados, intervalo_funcionarios, nome_planilha
        )
    ...
    def _carregar_dados(
        self, caminho_dados, intervalo_funcionarios, nome_planilha
    ):
        ...
        for i in range(*intervalo_funcionarios):
            ...
            valores = {
                ...
                "cpf": cpf,
                # 'dias': 30.,
                "dias": sheet[f"D{i}"].value,
                "salario_base": sheet[f"E{i}"].value,
                "salario_total": sheet[f"O{i}"].value,
                # 'Valor Adicional': sheet[f'G{i}'].value,
                "Valor Adicional": 0.0,
                "Valor Adicional Noturno": 0.0,
                # 'Valor Adicional Noturno': sheet[f'G{i}'].value,
                "Valor Reserva Técnica": 0.0,
                "Valor Encargos": sheet[f"F{i}"].value,
                "Valor Insalubridade": 0.0,
                # 'Valor Periculosidade': sheet[f'F{i}'].value,
                "Valor Periculosidade": 0.0,
                "Valor Outros": 0.0,
                "Valor Vale Transporte": sheet[f"I{i}"].value,
                # 'Valor Vale Transporte': 0.,
                ...
                "Valor Farda": 0.0,
                "Valor Munição": 0.0,
                "Valor Seguro Vida": 0.0,
                "Valor Supervisão": 0.0,
                "Valor Equipamento": 0.0,
                "Valor Plano Saúde": sheet[f"L{i}"].value,
                "Valor Intra jornada Diurno": 0.0,
                "Valor Intra jornada Noturno": 0.0,
                "Valor Tributos": sheet[f"M{i}"].value,
                "Valor Insumos de Mão de Obra": 0.0,
                "Quantidade Hora Extra": 0.0,
                "Valor Hora Extra": 0.0,
                "Valor DSR": 0.0,
                "Valor Extra Encargos": 0.0,
                "Valor Extra Taxa": 0.0,
                "Valor Extra Tributos": 0.0,
                "Quantidade Diarias": 0.0,
                "Valor Passagem": 0.0,
                "Valor Viagem": 0.0,
                "Valor Viagem Taxa": 0.0,
                "Valor Viagem Tributos": 0.0,
            }
    ...
    def _salario_total_compativel(self, funcionario):
        ...
        """
        return (
            funcionario.salario_total
            == self.dados[funcionario.cpf]["salario_total"]
        )
