...
class FuncionarioFatura(Funcionario):
    ...
    def _load_data(self):
        self.dias_trabalhados = self._load_atrib_value(
            self._loc_dias_trabalhados
        )
...
class DemaisInformacoes(TypeAgent):
    def __init__(self, webdriver):
        ...
        self._loc_aba_montanteA = (
            By.CSS_SELECTOR,
            "li.nav-item:nth-child(1) a",
        )
        self._loc_aba_montanteB = (
            By.CSS_SELECTOR,
            "li.nav-item:nth-child(2) a",
        )
        self._loc_aba_montanteC = (
            By.CSS_SELECTOR,
            "li.nav-item:nth-child(3) a",
        )
        self._loc_aba_provisionamento = (
            By.CSS_SELECTOR,
            "li.nav-item:nth-child(4) a",
        )

        self._loc_tabela_montanteA = (
            By.CSS_SELECTOR,
            "tab.tab-pane:nth-child(1) tbody",
        )
        self._loc_tabela_montanteB = (
            By.CSS_SELECTOR,
            "tab.tab-pane:nth-child(2) tbody",
        )
        self._loc_tabela_montanteC = (
            By.CSS_SELECTOR,
            "tab.tab-pane:nth-child(3) tbody",
        )
    ...
    def _preencher(self, loc_tabela, dados):
        ...
        for row, _loc_input in zip(
            tabela.find_elements(By.TAG_NAME, "tr"), _loc_inputs
        ):
    ...
    def _listar_loc_inputs(self, loc_tabela):
        complemento = " " + "input"
        num_inputs = len(
            self.find_elements((loc_tabela[0], loc_tabela[1] + complemento))
        )
